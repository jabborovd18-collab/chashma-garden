...
export function cronAuthorized(request) {
  // Vercel qiymatni so'zma-so'z saqlaydi — tirnoq bilan ko'chirilgan
  // bo'lsa uni olib tashlaymiz
  const xom = process.env.CRON_SECRET
  const kutilgan = xom ? xom.trim().replace(/^["']|["']$/g, '') : ''
